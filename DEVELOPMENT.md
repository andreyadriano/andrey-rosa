...
Guia prático pra rodar o projeto e mexer no conteúdo. Pra uma visão geral do que o site faz, veja o [README](README.md); pra decisões de arquitetura e convenções internas, veja [AGENTS.md](AGENTS.md).
...
2. No topo de cada arquivo, exporte os metadados como um objeto JS (⚠️ **não** use frontmatter em YAML com `---` — veja o porquê em [AGENTS.md](AGENTS.md)):
...
O "sistema de arquivos" que o terminal e o Explorador de Arquivos navegam é lido de verdade em disco, a partir de [`public/vfs/`](public/vfs) — não tem dado hardcoded em código. Ver a seção correspondente em [AGENTS.md](AGENTS.md) pra convenção de `.txt`/`.link`/imagens.
